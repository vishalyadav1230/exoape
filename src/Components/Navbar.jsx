...
function Navbar() {
  ...
  return (
    <div className="w-full fixed z-[999]">
      <div className="max-w-screen-2xl px-5 py-5 sm:py-12 sm:px-14 md:py-8 md:px-20 mx-auto flex items-center justify-between text-white">
        {/* Logo */}
        <motion.div
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          transition={{ ease: Power4.easeInOut, duration: 0.9 }}
          className="logo w-24 md:w-32"
        >
          {/* Your SVG logo here */}
          <svg
            viewBox="0 0 95 25"
        ...
        <motion.span
          ...
          whileInView={{ opacity: 1 }}
          transition={{ ease: Power4.easeInOut, duration: 0.9 }}
          className="sm:hidden flex gap-2 cursor-pointer"
          onClick={toggleMenu}
        >
          <p>Menu</p>
          <IoMenuSharp size={"1.5em"} />
        </motion.span>
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1 }}
          transition={{ ease: Power4.easeInOut, duration: 0.9 }}
          className="links hidden sm:flex gap-10 md:gap-12"
        >
          {["Work", "Studio", "News", "Contact"].map((item, index) => (
            <a
              key={index}
              className="relative text-md font-semibold cursor-pointer group"
            >
              {item}
              <span className="absolute left-0 bottom-0 h-[2px] mt-2 w-0 bg-white transition-all duration-500 group-hover:w-full"></span>
            </a>
          ))}
        </motion.div>
      </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="fixed top-0 left-0 w-full h-screen bg-black text-white flex flex-col items-center justify-center gap-10 z-[998]"
          >
            {/* ✅ Close Button */}
            <button
              onClick={toggleMenu}
